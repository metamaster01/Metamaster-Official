// lib/supabase/admin-blogs.ts
// All admin-side Supabase operations for blogs
// These use the anon key but rely on RLS + admin role check
...
// ─── List all blogs (admin sees all statuses) ─────────────────────────────────
...
// ─── Get single blog with sections for edit form ──────────────────────────────
...
export async function adminCreateBlog(
  ...
  if (tagIds.length > 0) {
    const { error: tagError } = await supabase
      .from('blog_tags')
      .insert(tagIds.map((tag_id) => ({ blog_id: blog.id, tag_id })))
...
export async function adminUpdateBlog(
  ...
  // 2. Replace sections — delete old, insert new
  ...
  if (tagIds.length > 0) {
    const { error: tagError } = await supabase
      .from('blog_tags')
      .insert(tagIds.map((tag_id) => ({ blog_id: id, tag_id })))
    if (tagError) throw new Error(tagError.message)
  }
}
...
export async function adminDeleteBlog(id: string) {
  const supabase = getAdminClient()
  // sections + tags cascade delete automatically (foreign key ON DELETE CASCADE)
  const { error } = await supabase.from('blogs').delete().eq('id', id)
  if (error) throw new Error(error.message)
}

// ─── Get all tags (for tag selector in form) ──────────────────────────────────
...
export async function adminCreateTag(name: string) {
  ...
  const { data, error } = await supabase
    .from('tags')
    .insert({ name, slug, color_hex: '#7c3aed' })
    .select('id, name, slug, color_hex')
    .single()
  if (error) throw new Error(error.message)
  return data
